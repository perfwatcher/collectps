...
var httpcfg = {
    ...
    path: undefined,
    configDir: undefined,
    cpsVersion: undefined,
    cpsHTTPUser: undefined,
    cpsHTTPPassword: undefined,
    plugins: undefined
};
...
exports.start = function() {
    ...
    app.use(basicAuth(function(credentials, req, res, next) {
        if (credentials.username != httpcfg.cpsHTTPUser || md5(credentials.password) != httpcfg.cpsHTTPPassword) {
            res.statusCode = 401;
    ...
    app.get('/collectps.css', function(req, res) {
        res.set('Content-Type', 'text/css');
        res.send(fs.readFileSync(httpcfg.path + '\\frontend\\collectps.css'));
    });
    
    app.get('/version', function(req, res) {
        res.set('Content-Type', 'application/json');
        res.json({ version: httpcfg.cpsVersion    });
    });
    ...
    app.get('/collectps_pid', function(req, res) {
        res.set('Content-Type', 'application/json');
        res.json({ collectps_pid: process.pid    });
    });
    
    app.get('/collectps_network', function(req, res) {
        var netconf = [];
    ...
    app.get('/httpconfig/port', function(req, res) {
        res.set('Content-Type', 'application/json');
        res.json({ cpsHTTPPort: (httpcfg.cfg.get('HttpConfig.listenPort') || 28230) });
    });
    ...
    app.post('/httpconfig/account', function(req, res) {
        ...
        if(        typeof req.body.user != 'undefined'
            ...
            httpcfg.cpsHTTPUser = req.body.user;
            httpcfg.cpsHTTPPassword = md5(req.body.password);
            cw_config_update({ 'HttpConfig': {'login' : httpcfg.cpsHTTPUser }});
    ...
    var server = app.listen(httpcfg.cfg.get('HttpConfig.listenPort') || 28230);
};
...
exports.init = function(c) {
    ...
    httpcfg.cpsVersion = c.cpsVersion;
    httpcfg.cpsHTTPUser = httpcfg.cfg.get('HttpConfig.login');
    httpcfg.cpsHTTPPassword = md5(httpcfg.cfg.get('HttpConfig.password'));
